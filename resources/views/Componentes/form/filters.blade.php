...
<div class="w-100p flex-row p-2 gap-2 just-end items-center" style="position: relative;">
    <div class="flex-row items-center gap-2" style="position: relative;">
        @if ($hasActiveFilters)
            <button id="clear-filters" class="rounded btn_blue" style="color: white; background-color: red;"
                title="Quitar filtros" data-route="{{ route($url) }}">
        ...
        <form action="{{ route($url) }}" id="filters" class="none w-100p rounded bg-white"
            ...
            <div class="flex just-end gap-3 w-100p p-2">
                {!! $form->select('campo', 'Criterio:', 'label-input-y-100', $filters, $fields, []) !!}
                {!! $form->text('filtro', 'Busqueda:', 'label-input-y-100', $filters) !!}
