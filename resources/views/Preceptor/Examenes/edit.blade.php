...
@section('content')
    ...
    <p class="w-100p">
        ...
        <a
            href="/preceptor/alumnos/{{ $examen->alumno->id }}/edit">{{ $examen->alumno->apellido }}{{ $examen->alumno->nombre }}</a>/
        Examen/
    ...
    <div class="edit-form-container">
        <div class="perfil_one br">
            ...
            <div class="perfil__info">
                <form method="post" action="{{ route('preceptor.examenes.update', ['examen' => $examen->id]) }}">
                    @csrf
                    @method('put')
                    ...
                    @else
                        <div class="campo_info3 font-400 border-none">
                            <label>No hay informacion de la mesa, esto es debido a que cuando se registro la inscripcion, no
    ...
    @if (!$config['modo_seguro'])
        <div class="upd">
            ...
            <form class="form-eliminar" c method="post"
                action="{{ route('preceptor.examenes.destroy', ['examen' => $examen->id]) }}">
                @csrf
                @method('delete')
