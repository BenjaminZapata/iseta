    ...
    <div class="edit-form-container">
        <div class="perfil_one br">
            ...
            <div class="perfil__info">
                ...
                <form method="post" action="{{route('Regente.mesas.update', ['mesa' => $mesa->id])}}">
                    @csrf
                    @method('put')
                    ...
                    <div class="perfil_dataname">
                        ...
                        <select class="campo_info rounded" name="prof_presidente">
                            ...
                            @foreach($profesores as $profesor)
                                <option @selected($mesa->prof_presidente != 0 && $mesa->profesor->id == $profesor->id)
                                    value="{{$profesor->id}}">
                    ...
                    <div class="perfil_dataname">
                        ...
                        <select class="campo_info rounded" name="prof_vocal_1">
                            ...
                            @foreach($profesores as $profesor)
                                <option @selected($mesa->prof_vocal_1 != 0 && $mesa->vocal1->id == $profesor->id)
                                    value="{{$profesor->id}}">
                                    {{$profesor->apellidoNombre()}}
                                </option>
                            @endforeach
                    ...
                    <div class="perfil_dataname">
                        ...
                        <select class="campo_info rounded" name="prof_vocal_2">
                            ...
                            @foreach($profesores as $profesor)
                                <option @selected($mesa->prof_vocal_2 != 0 && $mesa->vocal2 && $mesa->vocal2->id == $profesor->id)
                                    value="{{$profesor->id}}">
                                    {{$profesor->apellidoNombre()}}
                    ...
                    <div class="botones-derecha"
                        ...
                        <a href="{{route('Regente.mesas.index')}}" style="display: flex; align-items: center;">
        ...
        <div class="perfil_one br">
            ...
            <div class="matricular">
                @if(true || strtotime($mesa->fecha) > time())
                    <p class="py-2">Estos alumnos han aprobado la cursada de esta materia, luego se volvera a validar sobre
                        correlativas y tiempos</p>

                    <form method="POST" action="{{route('Regente.examenes.store')}}">
                        @csrf
        ...
        <div class="table">
            <div class="table__header">
                ...
                <div class="flex just-center">
                    <a href="{{route('Regente.mesas.acta', ['mesa' => $mesa->id])}}" target="_blank"><button
                            class="btn_grey">Regular</button></a>
                    <a href="{{route('Regente.mesas.actaprom', ['mesa' => $mesa->id])}}" target="_blank"><button
                            class="btn_grey">Promoción</button></a>
                    <a href="{{route('Regente.mesas.actalibre', ['mesa' => $mesa->id])}}" target="_blank"><button
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <form action="{{route('admin.examenes.nota', ['examen' => $examen->id])}}" method="POST">
                                    @csrf
                                    <input name="nota" placeholder="0 = sin rendir, a = ausente" class="p-1 rounded border-1"
                                        value="{{$examen->nota()}}">
